...
class HomeView(BaseView):
    def get(self, request):
        ...
        self.views['wish_counts'] = count_wishlist(request)
...
class WishlistView(BaseView):
    def get(self, request):
        ...
        self.views['cart_counts'] = count_cart(request)
        username = request.user.username
        self.views['wishlist_view'] = Wishlist.objects.filter(username = username)
...
def add_to_wishlist(request, slug):
    ...
    if Product.objects.filter(slug = slug).exists():
        if Wishlist.objects.filter(slug = slug, username = username).exists():
            messages.error(request, 'The product already exists')
        else:
